...
(function ($) {
  Drupal.behaviors.testing_glass = {
    attach: function(context) {
        $('#testing-glass-tabs a').click(function (e) {
            e.preventDefault();
            $(this).tab('show');
        });
        $('#testing-glass-vert-tabs a').click(function (e) {
            e.preventDefault();
            $(this).tab('show');
        });

        if ($('.dd-transformed-door').is(':enabled')) {
            $('.dd-transformed-door').ddslick({
                width:292,
                background: '#fff',
                onSelected: function(selectedData){
                    if (selectedData.selectedData.value) {
                        $('.dd-value-door').val(selectedData.selectedData.value).attr('selected', 'selected').trigger('change');
                    }
               }
           });
        }

       if ($('.dd-transformed-sidelite').is(':enabled')) {
            $('.dd-transformed-sidelite').ddslick({
                width:292,
               background: '#fff',
               onSelected: function(selectedData){
                   if (selectedData.selectedData.value) {
                       $('.dd-value-sidelite').val(selectedData.selectedData.value).attr('selected', 'selected').trigger('change');
                   }
              }
          });
        }
